...
namespace BSS {
    ...
    public class EditorSpriteAnimation : EditorWindow
    {
        ...
        [MenuItem("Tools/BSS/Animation/Sprite Animation Editor")]
        private static void ShowWindow()
        {
            GetWindow(typeof(EditorSpriteAnimation), false, "Sprite Animation");
        }

        //[MenuItem("Assets/Create/BSS/Sprite Aniamtion(Selected)")]
        ...
        private void InitializeReorderableList()
        {
            ...
            for (int i = 0; i < selectedAnimation.FramesCount; i++)
                frames.Add(new AnimationFrame(selectedAnimation.animFrames[i].frame, selectedAnimation.animFrames[i].duration));
        ...
        private void NewAnimationBox()
        {
			EditorGUILayout.BeginHorizontal(EditorStyles.toolbar, GUILayout.ExpandWidth(true));
            {
                ...
                // New animaton button
                if (GUILayout.Button("Apply Animation", EditorStyles.toolbarButton, GUILayout.ExpandWidth(false))) {
                    ApplyAnimation();
                }
        ...
        private void DrawFrameListElement(Rect r, int i, bool active, bool focused)
        {
            ...
            {
                Sprite selectFrame = selectedAnimation.animFrames[i].frame;
                string spriteName = (selectFrame != null) ? selectFrame.name : "No sprite selected";
                EditorGUI.LabelField(new Rect(r.x, r.y + 2, r.width, r.height), spriteName);
                selectFrame = EditorGUI.ObjectField(new Rect(r.x + r.width - 120, r.y + 1, 50, r.height - 4), "", selectFrame, typeof(Sprite), false) as Sprite;
                EditorGUIUtility.labelWidth = 20;
                selectedAnimation.animFrames[i].duration = EditorGUI.IntField(new Rect(r.x + r.width - 50, r.y + 1, 50, r.height - 4), speedScaleIcon, selectedAnimation.animFrames[i].duration);
            }
        ...
        private void RemoveFrameListItem(ReorderableList list)
        {
            ...
            int i = list.index;
            selectedAnimation.animFrames.RemoveAt(i);
        ...
        private void ReorderFrameListItem(ReorderableList list)
        {
            ...
            Sprite s = selectedAnimation.animFrames[frameListSelectedIndex].frame;
            selectedAnimation.animFrames.RemoveAt(frameListSelectedIndex);
            selectedAnimation.animFrames.Insert(list.index, new AnimationFrame(s, 1));
        ...
        private void AddFrame()
        {
            ...
            selectedAnimation.animFrames.Add(new AnimationFrame(null, 1));
        }
        ...
        {
            AddFrame();
            selectedAnimation.animFrames[selectedAnimation.animFrames.Count - 1] = new AnimationFrame(s, 1);
            frameList.list[selectedAnimation.animFrames.Count - 1] = new AnimationFrame(s, 1);
        }
