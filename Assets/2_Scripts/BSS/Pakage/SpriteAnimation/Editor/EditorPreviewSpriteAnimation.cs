...
namespace BSS {
    ...
    public class EditorPreviewSpriteAnimation : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            {
                sr.sprite = animation.animFrames[0].frame;
        ...
        private void Update()
        {
            ...
            if (isPlaying)
            {
                ...
                if (1f / framesPerSecond < animationTimer)
                {
                    ...
                    animationTimer = 0;
                    if (frameDurationCounter >= animation.animFrames[currentFrame].duration)
                    {
   ...
   //     }
        ...
        public override void OnInteractivePreviewGUI(Rect r, GUIStyle background)
        {
            ...
            {
                // Draw Camera
                sr.sprite = animation.animFrames[currentFrame].frame;
                go.SetActive(true);
        ...
        private void InitializeReorderableList()
        {
            ...
            for (int i = 0; i < animation.FramesCount; i++)
                frames.Add(new AnimationFrame(animation.animFrames[i].frame, animation.animFrames[i].duration));
        ...
        private void DrawFrameListElement(Rect r, int i, bool active, bool focused)
        {
            ...
            if (i < animation.FramesCount)
            {
                ...
                string spriteName = (animation.animFrames[i].frame != null) ? animation.animFrames[i].frame.name : "No sprite selected";
                EditorGUIUtility.labelWidth = r.width - 105;
                animation.animFrames[i].frame = EditorGUI.ObjectField(new Rect(r.x + 10, r.y + 1, r.width - 85, r.height - 4), spriteName, animation.animFrames[i].frame, typeof(Sprite), false) as Sprite;

                EditorGUIUtility.labelWidth = 20;
                animation.animFrames[i].duration = EditorGUI.IntField(new Rect(r.x + r.width - 50, r.y + 1, 50, r.height - 4), speedScale, animation.animFrames[i].duration);
        ...
        private void AddFrame()
        {
            ...
            animation.animFrames.Add(new AnimationFrame(null, 1));
        }
        ...
        private void ReorderFrameListItem(ReorderableList list)
        {
            ...
            animation.animFrames.RemoveAt(frameListSelectedIndex);
            animation.animFrames.Insert(list.index, animation.animFrames[frameListSelectedIndex]);
